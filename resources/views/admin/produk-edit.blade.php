...
<form action="{{ route('admin.produk.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf 
    @method('PUT') {{-- ── WAJIB DI LARAVEL UNTUK METHOD UPDATE DATA ── --}}
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-2xl p-4 text-center hover:border-[#C81010] transition-all group">
                    ...
                    @if($product->gambar)
                        <div class="flex justify-center mb-3">
                            <img src="{{ asset('storage/products/' . $product->gambar) }}" class="w-32 h-32 object-cover rounded-xl shadow-sm border">
                        </div>
                    @endif
                    ...
                    <p class="text-[10px] text-gray-400 mt-2">Biarkan kosong jika tidak ingin mengubah gambar.</p>
                </div>
            </div>
